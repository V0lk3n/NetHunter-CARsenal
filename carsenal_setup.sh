...
setup() {
    ...
    sudo apt install -y autoconf cmake expect fluxbox git libconfig-dev libsdl2-dev libsdl2-image-dev libsdl2-ttf-dev libsocketcan-dev make maven meson python3-pip python3-venv websockify x11vnc xserver-xephyr xvfb python3-can metasploit-framework
    ...
    checkAndInstall "Freediag" "/usr/local/bin/freediag" "/opt/car_hacking/freediag" \
        "cd /opt/car_hacking && sudo git clone https://github.com/v0lk3n/freediag.git && cd freediag && ./build_simple.sh && sudo rm /usr/local/bin/freediag && sudo rm /usr/local/bin/diag_test && sudo ln -s /opt/car_hacking/freediag/build/scantool/freediag /usr/local/bin/freediag && sudo ln -s /opt/car_hacking/freediag/build/scantool/diag_test /usr/local/bin/diag_test && wget https://raw.githubusercontent.com/V0lk3n/NetHunter-CARsenal/refs/heads/main/freediag.ini -O /opt/car_hacking/freediag/build/scantool/freediag.ini"
...
update() {
    ...
    checkAndUpdate "Freediag" "/usr/local/bin/freediag" "/opt/car_hacking/freediag" \
        "cd /opt/car_hacking/freediag && old=\$(git rev-parse HEAD) && sudo git pull && new=\$(git rev-parse HEAD) && [[ \"\$old\" != \"\$new\" ]] && { echo 'Update detected! Updating...'; ./build_simple.sh && sudo rm /usr/local/bin/freediag && sudo rm /usr/local/bin/diag_test && sudo ln -s /opt/car_hacking/freediag/build/scantool/freediag /usr/local/bin/freediag && sudo ln -s /opt/car_hacking/freediag/build/scantool/diag_test /usr/local/bin/diag_test && wget https://raw.githubusercontent.com/V0lk3n/NetHunter-CARsenal/refs/heads/main/freediag.ini -O /opt/car_hacking/freediag/build/scantool/freediag.ini; }"
