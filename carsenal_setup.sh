...
setup() {
    ...
    sudo apt install -y autoconf can-utils cmake expect fluxbox git libconfig-dev libsdl2-dev libsdl2-image-dev libsdl2-ttf-dev libsocketcan-dev make maven meson python3-pip python3-venv websockify x11vnc xserver-xephyr xvfb python3-can metasploit-framework
    ...
    checkAndInstall "Freediag" "/usr/local/bin/freediag" "/opt/car_hacking/freediag" \
        "cd /opt/car_hacking && sudo git clone https://github.com/v0lk3n/freediag.git && cd freediag && ./build_simple.sh && sudo ln -sf /opt/car_hacking/freediag/build/scantool/freediag /usr/local/bin/freediag && sudo ln -sf /opt/car_hacking/freediag/build/scantool/diag_test /usr/local/bin/diag_test && wget -N https://raw.githubusercontent.com/V0lk3n/NetHunter-CARsenal/refs/heads/main/freediag.ini -O /opt/car_hacking/freediag/build/scantool/freediag.ini"
    ...
    grep -q '#noVNC_control_bar' /opt/noVNC/app/styles/base.css && \
    sed -n '/#noVNC_control_bar[[:space:]]*{/,/}/p' /opt/noVNC/app/styles/base.css | grep -q 'display[[:space:]]*:[[:space:]]*none;' || \
...
update() {
    ...
    sudo apt install -y autoconf can-utils cmake expect fluxbox git libconfig-dev libsdl2-dev libsdl2-image-dev libsocketcan-dev make maven meson python3-pip python3-venv websockify x11vnc xserver-xephyr xvfb python3-can metasploit-framework
    ...
    checkAndUpdate "Freediag" "/usr/local/bin/freediag" "/opt/car_hacking/freediag" \
        "cd /opt/car_hacking/freediag && old=\$(git rev-parse HEAD) && sudo git pull && new=\$(git rev-parse HEAD) && [[ \"\$old\" != \"\$new\" ]] && { echo 'Update detected! Updating...'; ./build_simple.sh && sudo ln -sf /opt/car_hacking/freediag/build/scantool/freediag /usr/local/bin/freediag && sudo ln -sf /opt/car_hacking/freediag/build/scantool/diag_test /usr/local/bin/diag_test && wget -N https://raw.githubusercontent.com/V0lk3n/NetHunter-CARsenal/refs/heads/main/freediag.ini -O /opt/car_hacking/freediag/build/scantool/freediag.ini; }"
